...
// === OBTENER TODOS LOS EVENTOS (OPCIONAL: FILTRAR POR FECHA O TODOS FUTUROS) ===
...
export async function agregarEvento(data) {
  ...
  if (FechaHoraInicio >= FechaHoraFin) throw new Error('Fecha de inicio debe ser anterior a la fin');
...
export async function actualizarEvento(id, data) {
  ...
  await conexion.execute(
    `UPDATE EVENTOS SET Asunto=?, Detalles=?, FechaHoraInicio=?, FechaHoraFin=?, ID_cliente=?, ID_proyecto=?, Otro=?
     WHERE ID_evento=?`,
